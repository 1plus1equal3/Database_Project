...
function generateClassList(Class_list) {
    ...
    Class_list.forEach(function (item, index) {
        if (index >= itemPerPage * currentPage && index < itemPerPage * (currentPage + 1)) {
            ...
            linkElement.href = "#" + (index + 1); // Append the ID correctly - Link to the test interface
            linkElement.style = "text-decoration: none; color: black;";
            cardBodyElement.innerHTML = `
                <div class="ml-3">
                    <strong>Test:</strong> ${item.name} <br>
                    <strong>Date Created:</strong> ${item.ID} <br>
                </div>`;
            var resultBtn = document.createElement("a");
            //Link to the test result interface
            resultBtn.innerHTML = `<a href="#" class="btn btn-info btn-icon-split">
                                        <span class="icon text-white-50">
                                            <i class="fas fa-info-circle"></i>
                                        </span>
                                        <span class="text">Test's result</span>
                                    </a>`;
            // Append the card body to the link
            linkElement.appendChild(cardBodyElement);
            // Append the link to the card
            cardElement.appendChild(linkElement);
            // Append the result button to the card
            cardElement.appendChild(resultBtn);
